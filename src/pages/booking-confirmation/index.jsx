...
// Assuming these are the paths to your local assets
        ...
        <div className="p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col md:flex-row md:justify-between md:items-start gap-6 sm:gap-8 md:gap-8 lg:gap-6">
          ...
          <div className="flex-1 min-w-0 md:min-w-0">
            <div className="flex items-center gap-2 sm:gap-3 md:gap-3 mb-5 sm:mb-6 md:mb-8">
              <div className="bg-[#70B42D] rounded-full p-1 shrink-0">
                <CheckCircle size={32} className="text-white fill-[#70B42D] w-8 h-8 shrink-0" />
              </div>
          ...
          <div className="flex flex-col items-center md:items-center md:shrink-0 gap-4">
            {/* Outlined Date Box */}
            <div className="border-[1.5px] border-[#70B42D] rounded-xl p-3 sm:p-4 md:p-4 w-full max-w-[9rem] sm:w-36 md:w-40 text-center">
              <p className="text-[#70B42D] text-4xl sm:text-[44px] md:text-[48px] font-bold leading-none">26</p>
              <p className="text-[#70B42D] text-base sm:text-[18px] md:text-[20px] font-medium">February</p>
              <p className="text-[#70B42D] text-xs sm:text-[13px] md:text-[14px] mt-1">8:30 PM</p>
              <div className="h-[1px] bg-[#70B42D] my-2 w-full"></div>
              <p className="text-[#70B42D] text-[9px] sm:text-[10px] uppercase font-bold">
                Booking Reference:
              </p>
              <p className="text-[#70B42D] text-[11px] sm:text-[12px]">BRK-458921</p>
            </div>

            {/* Price Display */}
            <div className="text-center">
              <p className="text-[#70B42D] text-2xl sm:text-[28px] md:text-[32px] font-bold">$90 Paid</p>
            </div>
          </div>
        </div>

        {/* Bottom Green Banner */}
        <div className="bg-[#A3D139] min-h-[100px] sm:h-[110px] md:h-[120px] w-full flex items-center px-4 sm:px-6 md:px-10 lg:px-12 py-6 sm:py-0 relative overflow-hidden">
          {/* Logo/Text Section */}
          <div className="flex items-center gap-3 sm:gap-4 z-10">
            {/* The Black Car Icon/Logo Image */}
            <img 
              src={blackCarImg} 
              alt="Car Logo" 
              className="w-12 sm:w-14 md:w-16 h-auto brightness-0"
            />
            {/* The Black "Crent" Text Image */}
            <img 
              src={blackCrent} 
              alt="Crent" 
              className="h-8 sm:h-9 md:h-10 w-auto brightness-0"
            />
          </div>          
        </div>
      </div>
